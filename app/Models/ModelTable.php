...
class ModelTable extends Model
{
    ...
    public static function bankBalance($id=null, $fromDate=null, $toDate= null)
    {
        return DB::select("SELECT b.*,
                                ( SELECT IFNULL(SUM(sm.paid), 0)
                                    FROM sales AS sm
                                        WHERE sm.payment_type = 'bank' 
                                        AND sm.account_id = b.id 
                                        " . ($fromDate==null ? "" : " AND sm.date BETWEEN '$fromDate' AND '$toDate' ") . "
                                    ) AS saleamount,
                                ( SELECT IFNULL(SUM(pm.paid), 0)
                                    FROM purchases AS pm
                                        WHERE pm.payment_type = 'bank' 
                                        AND pm.account_id = b.id
                                ...
                                ( SELECT IFNULL(SUM(trb.amount), 0)
                                    FROM transactions AS trb
                                        WHERE trb.transaction_type = 'deposit' 
                                        AND trb.account_id = b.id 
                                        " . ($fromDate==null ? "" : " AND trb.date BETWEEN '$fromDate' AND '$toDate' ") . "
                                    ) AS depositamount,
                                ( SELECT IFNULL(SUM(trbw.amount), 0)
                                    FROM transactions AS trbw
                                        WHERE trbw.transaction_type = 'withdraw' 
                                        AND trbw.account_id = b.id 
                                        " . ($fromDate==null ? "" : " AND trbw.date BETWEEN '$fromDate' AND '$toDate' ") . "
                                    ) AS withdrawamount,
                                ( SELECT
                                ( saleamount + customeramount + depositamount ) -( purchaseamount + supplieramount + withdrawamount )
                                ) AS totalbalance
                            FROM bank_accounts b
                            WHERE 1 = 1 " . ($id==null ? "" : " AND b.id = '$id' ") . "
                        ");
    }
}
